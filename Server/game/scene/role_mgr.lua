...
function role_mgr:init_archetype(  )
	self.role_archetype = self.entity_mgr:CreateArchetype({
		"umo.position", "umo.target_pos", "umo.uid", "umo.type_id", "umo.hp", "umo.scene_obj_type", "umo.move_speed", "umo.aoi_handle", 
	})
end

function role_mgr:create_role( uid, role_id, pos_x, pos_y, pos_z, aoi_handle )
	...
	self.entity_mgr:SetComponentData(role, "umo.move_speed", {value=100})
	self.entity_mgr:SetComponentData(role, "umo.aoi_handle", {value=aoi_handle})
	return role
end
