...
namespace Component
{
    ...
    public class SpeedData : MonoBehaviour
    {
        ...
        private Dictionary<string, int> bod;
        ...
        public SpeedData()
        {
            curSpeed = 0;
            baseSpeed = 0;
            bod = new Dictionary<string, int>();
        }

        public void InitSpeed(int baseSpeed)
        {
        ...
        offset：变更万分比，0时即不动
        */
        public void ChangeSpeed(string bodName, bool isSet, int offset)
        {
            if (isSet)
                bod[bodName] = offset;
            else
