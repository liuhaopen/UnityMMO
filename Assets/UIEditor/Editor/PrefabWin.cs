...
public class PrefabWin : EditorWindow
{
	...
	void DestroyTexture (Item item)
	{
		...
		{
            Debug.Log("DestroyTexture():" + new System.Diagnostics.StackTrace().ToString());
	...
	void GeneratePreview (Item item, bool isReCreate = true)
	{
		if (item == null || item.prefab == null) return;
		{
            ...
            if (!isReCreate && File.Exists(preview_path))
            {
                ...
                Debug.Log("GeneratePreview() not isReCreate:" + (texture!=null).ToString() + " stack : " + new System.Diagnostics.StackTrace().ToString());
                item.tex = texture;
            }
            else
            {
                ...
                {
                    DestroyTexture(item);
                    Debug.Log("GeneratePreview() isReCreate:" + (Tex != null).ToString() + " stack : " + new System.Diagnostics.StackTrace().ToString());
                    item.tex = Tex;
	...
	void OnGUI ()
	{
        ...
        mPos = EditorGUILayout.BeginScrollView(mPos);
		{
			...
			for (int i = 0; i < indices.size; ++i)
			{
				...
				{
                    if (ent.tex == null)
                        Debug.Log("prefab preview texture empty!");
					if (ent.tex != null)
					{
