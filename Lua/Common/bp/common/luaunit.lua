...
local function mismatchFormattingPureList( table_a, table_b )
    ...
    * result: if success is true, a multi-line string with deep analysis of the two lists
    ...
    local function insertABValue(bp, bi)
        bi = bi or bp
        if is_equal( table_a[bp], table_b[bi]) then
            return extendWithStrFmt( result, '  = A[%d], B[%d]: %s', bp, bi, prettystr(table_a[bp]) )
        else
            extendWithStrFmt( result, '  - A[%d]: %s', bp, prettystr(table_a[bp]))
